...
def topology_bfs(graph, indegree_array, output_topology, queue):
    ...
    for x in range(len(indegree_array)):
        if x > 0 and indegree_array[x] == 0 and x not in output_topology and x not in queue:
            heappush(queue, x)

    vertex = heappop(queue)

    for z in graph[vertex][0]:
        indegree_array[z] -= 1
...
read_list = [["input2a.txt", "output2a.txt"], ["input2b.txt", "output2b.txt"], ["input2c.txt", "output2c.txt"]]
